...
if (isset($_GET['cat_id'])) {
   //getting the category information
    ...
    $query = "SELECT * FROM ecategory WHERE id = {$category_id} LIMIT 1";
    ...
    if($result_set){
      if (mysqli_num_rows($result_set)==1) {
        ...
        //category not found
        header('Location:ecategory.php?err=ecategory_not_found');
      }
    }else{
      //query failed
      header('Location:ecategory.php?err=query_failed');
    }
...
if (isset($_POST['category_submit'])) {
  ...
  $query = "SELECT * FROM ecategory WHERE category = '{$cat}' /*AND id != {$cat_id}*/ LIMIT 1";

  $result_set = mysqli_query($connection, $query);

  if ($result_set) {
    if (mysqli_num_rows($result_set)==1) {
      $errors[] = 'eCategory already exists.';
    }
  }
  ...
  if (empty($errors)) {
    ...
    //query goes here
    $update_cat = "UPDATE ecategory SET category = '{$cat_name}' WHERE id = {$cat_id} LIMIT 1";
    ...
    if ($result) {
      ...
      header('Location:ecategory.php?update=true');
    }else{
...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <a href="ecategory.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-chevron-circle-left fa-sm text-white-50"></i> Back to Categories</a>
  </div>

  <div class="row">
    <div class="col-12">
      <form method="POST" action="edit-ecategory.php">
